 ...
 * Uses actual user UUID from database with Google Calendar integration
 */
...
async function extractRealUserUUID() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.error('❌ No users found with Google Calendar integration');
      console.log('Please ensure at least one user has connected Google Calendar');
      
      // Fallback: Try to get any user without Google Calendar for testing
      console.log('🔄 Attempting to find any user for testing...');
      ...
      console.log(`⚠️  Using fallback user (no Google Calendar): ${fallbackUser.email}`);
      console.log(`📋 User ID: ${fallbackUser.id}`);
      console.log('⚠️  Note: This user may not have Google Calendar integration');
    ...
    console.log(`✅ Found user with Google Calendar integration: ${user.email}`);
...
async function runRealUserTests() {
  ...
  console.log('📋 Querying database for users with Google Calendar integration');
  ...
  if (!realUserId) {
    console.error('❌ Cannot proceed without user UUID from database');
    console.log('Please ensure there is at least one user in the database');
    return false;
  }
  
  console.log('✅ User UUID extracted successfully');
  console.log('='.repeat(80));
