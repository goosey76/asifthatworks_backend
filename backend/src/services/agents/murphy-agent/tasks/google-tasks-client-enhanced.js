...
class GoogleTasksClient {
   ...
   * @param {string} userId - User ID for task access
   * @param {string} tasklistId - Task list ID (defaults to @default)
   * @param {Array} taskOperations - Array of task operations
   * @returns {Promise<object>} Batch operation results
   */
  async batchTaskOperations(userId, tasklistId = '@default', taskOperations) {
    const results = {
      successful: [],
      failed: []
    };
    ...
    for (const operation of taskOperations) {
      try {
        ...
        switch (operation.action) {
          case 'create':
            const createResult = await tasks.tasks.insert({
              tasklist: tasklistId,
              requestBody: operation.taskResource
            });
