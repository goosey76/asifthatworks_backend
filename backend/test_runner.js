...
/**
 * Test Runner for Comprehensive Multi-Agent Task Management Test Suite
 * 
 * Usage:
 *   node test_runner.js                    // Run all 30 tests
 *   node test_runner.js --agent JARVI      // Run only JARVI tests (10 cases)
 *   node test_runner.js --agent GRIM --case 5  // Run single test case
 *   node test_runner.js --config custom.json   // Use custom configuration
 *   node test_runner.js --help             // Show help
 */

const TestSuite = require('./comprehensive_30_agent_test_suite');
const fs = require('fs');
...
class TestRunner {
  ...
  showHelp() {
    console.log(`
Comprehensive Multi-Agent Test Suite Runner
...
process.on('uncaughtException', (error) => {
  ...
  if (runner && runner.args && runner.args.verbose) {
    console.error(error.stack);
  }
  process.exit(1);
});
